...
const App: React.FC = () => {
  ...
  const saveFoodItem = async (item: FoodItem) => {
    ...
    // If it's a random string generated by frontend, Supabase treats it as a new insert if we omit ID
    // or upsert if we pass it (but passing random string as UUID fails).
    // Strategy: If local state ID doesn't look like a UUID, assume new and let DB generate ID.
    const isTempId = item.id.length < 20; // UUIDs are 36 chars. Simple check.
